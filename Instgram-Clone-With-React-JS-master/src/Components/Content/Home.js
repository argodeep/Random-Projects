...
class Home extends React.Component {
  ...
  constructor(props) {
    super(props);
    this.posts = firebase.firestore().collection("posts").orderBy("postedOn", "desc").limit(20);
                  ...
                  <Grid container className={classes.grid} spacing={32} justify="center">
                        ...
                        { this.state.posts.map((data, i) => (   
                          <Card className={classes.card} key={i}>
                            <CardHeader
                              avatar={
                                <Link to={`/${data.userName}`}><Avatar alt="Icon" src={data.userProfileImg} style={{border: '1px solid rgb(239, 239, 239)'}} /></Link>
                          
                              }
                              title={<Link to={`/${data.userName}`} style={{ textDecoration: 'none', color: '#000' }}>{data.userName}</Link>}
                              style={{padding: '8px 16px'}}
                            />
                           <ProgressiveImage src={data.postImage} placeholder="/assets/placeholder.png" width='100%'>
                              {(src, loading) => <img src={src} onClick={() => this.handleClicks(data.key, data.postLike, data.postId)}  value={data.key} count={data.postLike} postid={data.postId} alt="post images"  width='100%' style={{opacity: loading ? 0.2 : 1, margin: 'auto', display: 'block'}} />}
                            </ProgressiveImage>                           

                            
                              <CardActions className={classes.actions} disableActionSpacing>
                                { likeMatch.includes(data.postId) === true  ? 
                                <IconButton aria-label="Added to favorites" style={{ padding: 8 }} value={data.key} count={data.postLike} postid={data.postId} onClick={() => this.handleDisLike(data.key, data.postLike, data.postId)}>
                                  <FavoriteIcon  style={{ color: 'red'}}/>
                                </IconButton> :
                                <IconButton aria-label="Add to favorites" style={{ padding: 8 }} value={data.key} count={data.postLike} postid={data.postId} onClick={() => this.handleLike(data.key, data.postLike, data.postId)}>
                              ...
                              className={classnames(classes.expand, {
                                ...
                                style={{ padding: 8 }}
                                postid={data.postId} 
                                onClick={() => this.handleExpandClick(data.postId)}>
                            ...
                            <CardContent style={{paddingTop: 0, paddingBottom: 0}}>
                              ...
                              <Typography component="p" style={{ display: 'inline', fontWeight: 'bold' }}>{data.postLike} {data.postLike > 1 ? 'Likes' : 'Like'}</Typography>
                              <div style={{ marginTop: 10, marginBottom: 10}}>
                              <Typography component="p" style={{ display: 'inline', fontWeight: 'bold'}}>{data.userName} </Typography>
                              <Typography component="p" style={{ display: 'inline'}}> {data.postDesc}</Typography>
                              </div>
                              <Typography component="p" style={{ display: 'inline', color: '#999'}}> {new Date(data.postedOn.toDate()).toDateString()}</Typography>
                              <Typography component="p" style={{color: 'blue'}}>
                                {data.hashtags}
                                  ...
                                  <CardContent style={{paddingTop: 0, paddingLeft: 0, paddingRight: 0, paddingBottom: 0 }}>
                                          { this.state.commentLogic === true && data.postId === this.state.idLogic && 
                                          ...
                                          <div>                                                
                                            ...
                                            { progress === true && data.postId === this.state.idLogic && 
                                                            <LinearProgress
                                                              style={{margin: 20}}
                                                              classes={{
                                            ...
                                            {  fetching === true && data.postId === this.state.idLogic ? 
                                            ...
                                            :
                                              ...
                                              { this.state.commentLogic === true && data.postId === this.state.idLogic && this.state.noComments === false &&
                                            ...
                                            <div style={{overflow: 'auto', maxHeight: '350px', paddingTop: 10, paddingLeft: 10, paddingRight: 10}}> 
                                              ...
                                              { this.state.commentloadArrayInital.map((data, i) => (
                                                  ...
                                                  <Typography component="p" style={{display: 'inline', fontWeight: 'bold'}}>{data.displayName} </Typography>
                                            ...
                                            { this.state.commentLogic === true && data.postId === this.state.idLogic && this.state.noComments === true && 

                                            <div>                                          

                                            { data.postId === this.state.idLogic && 
                      ...
                      <Grid item sm={12} md={3} lg={3}>
                            ...
                            <ListItem alignItems="flex-start" key={i} style={{paddingLeft: 0}}>
                                  <ListItemAvatar>
                                    <Avatar alt="profile image" src={data.profileImg} style={{height: '50px', width: '50px', marginTop: 0 }} />
                                  </ListItemAvatar>
                                  <ListItemText
                                    primary={
                                      <React.Fragment>
                                        <Link to={`/${data.profileUsername}`} style={{ flexGrow: 1, textDecoration: 'none' }}>
                                        <Typography component="span" style={{fontWeight: 500, fontSize: 14}}>
                                    ...
                                    secondary={
                                      <React.Fragment>
                                        <Typography component="span" style={{fontSize: 14, color: '#999'}}>
                                        {data.fullName}
                                        </Typography>
                                      </React.Fragment>
                                    }
                            ...
                            <Link to={`/create`} style={{ flexGrow: 1, textDecoration: 'none' }}>
                                <img src={'assets/add.jpg'} alt="default images" style={{width: '100%', marginBottom: 10, borderRadius: 5, display: 'block'}}/>
                            </Link>
